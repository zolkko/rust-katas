...
fn main() {
    ...
    let result = if mcomm.rank() == 0 {
        ...
        let data: Vec<Vec<_>> = (0..(mcomm.size() as usize)).map(|x| (0..(x + 2)).collect()).collect();

        let counts: Vec<_> = data.iter().map(|x| x.len() as Count).collect();
        let displs: Vec<_> = counts.iter().scan(0 as Count, |state, x| {
        ...
        mcomm.process_at_rank(0).scatter_into_root(&counts[..], &mut count);

        // Then scatter the data
        let mut result = vec![0usize; count as usize];
        let data = Partition::new(&flat_data[..], counts, displs);
        mcomm.process_at_rank(0).scatter_varcount_into_root(&data, &mut result);
        result
